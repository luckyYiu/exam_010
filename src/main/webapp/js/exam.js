...
function chooseHandler(chooseId) {
	...
	$.ajax({
		...
		url: path+"chooseInfo/choose.action",
...
function subTime() {
	...
	if(lastmin<=0 && lastsec<=1) {
		...
		zeroModal.show({
			...
			onClosed : function() {
				//考试结束回到考试中心页面
				location.href = path+"examPlan/willexams.action?classId="+$('#examEndRefresh_classId').text().trim()+"&gradeId="+$('#examEndRefresh_gradeId').text().trim()+"&studentId="+$('#exam-studentId').text().trim()+"";
			}
		});
		return false;
	}
