...
$(function() {
    ...
    $(".examPaper-subjects").click(function() {
        ...
        $("#afterTr").children("td").append("<table class='table' style='width:100%;'>" +
            ...
            "</tr>" +
            "</table>");
        $.getJSON(path+"examSubjectMiddle/getSubExamPar.action", "examPaperId="+$(this).attr("id"), function(data) {
            //试卷中没有试题
            if (data == null || data == "") {
                zeroModal.show({
                    title : "试卷为空",
            ...
            $.each(data, function(i, item) {
                ...
                var LENFTH_SUFFIX = "<a href=path+'subject/subjectList.action?subjectId='+"+subjectId+">[...]</a>";
...
function removeSubjectWithExamPaper(subjectId, examPaperId, score) {
	...
	$.ajax({
		...
		url: path+"examSubjectMiddle/removeSubjectFromPaper.action",
		success: function() {
			//修改试卷行的试题数量和总分信息
