				...
				<dl subject="1" style="width:100%;">
					...
					<c:if test="${esms != null }">
						...
						<c:forEach items="${esms }" var="esm">
								<dd style="float: left;display: none; width: 100%;" index="${index }" id="${esm.subjectId }">
									<div>
										<p>${index }、${esm.subjectName }</p>
									</div>
									<div>
										<%-- 单选 --%>
										<c:if test="${esm.subjectType == 0 }">
											<p class="aswer-p"><input class="aswer-option" id="${examPaperId }-${esm.subjectId }-${index }-A" type="radio" name="chooseRight-${esm.subjectId }" value="A"/>&nbsp;<span class="subjectOption">${esm.optionA }</span></p>
											<p class="aswer-p"><input class="aswer-option" id="${examPaperId }-${esm.subjectId }-${index }-B" type="radio" name="chooseRight-${esm.subjectId }" value="B"/>&nbsp;<span class="subjectOption">${esm.optionB }</span></p>
											<p class="aswer-p"><input class="aswer-option" id="${examPaperId }-${esm.subjectId }-${index }-C" type="radio" name="chooseRight-${esm.subjectId }" value="C"/>&nbsp;<span class="subjectOption">${esm.optionC }</span></p>
											<p class="aswer-p"><input class="aswer-option" id="${examPaperId }-${esm.subjectId }-${index }-D" type="radio" name="chooseRight-${esm.subjectId }" value="D"/>&nbsp;<span class="subjectOption">${esm.optionD }</span></p>
										</c:if>
										<%-- 多选 --%>
										<c:if test="${esm.subjectType == 1 }">
											<p class="aswer-p subject-id-${esm.subjectId }" data-sid="${esm.subjectId }" data-type="checkbox" data-val="${examPaperId }-${esm.subjectId }-${index }-"><input class="aswer-option" id="${examPaperId }-${esm.subjectId }-${index }-A" type="checkbox" name="chooseRight-${esm.subjectId }" value="A"/>&nbsp;<span class="subjectOption">${esm.optionA }</span></p>
											<p class="aswer-p subject-id-${esm.subjectId }" data-sid="${esm.subjectId }" data-type="checkbox" data-val="${examPaperId }-${esm.subjectId }-${index }-"><input class="aswer-option" id="${examPaperId }-${esm.subjectId }-${index }-B" type="checkbox" name="chooseRight-${esm.subjectId }" value="B"/>&nbsp;<span class="subjectOption">${esm.optionB }</span></p>
											<p class="aswer-p subject-id-${esm.subjectId }" data-sid="${esm.subjectId }" data-type="checkbox" data-val="${examPaperId }-${esm.subjectId }-${index }-"><input class="aswer-option" id="${examPaperId }-${esm.subjectId }-${index }-C" type="checkbox" name="chooseRight-${esm.subjectId }" value="C"/>&nbsp;<span class="subjectOption">${esm.optionC }</span></p>
											<p class="aswer-p subject-id-${esm.subjectId }" data-sid="${esm.subjectId }" data-type="checkbox" data-val="${examPaperId }-${esm.subjectId }-${index }-"><input class="aswer-option" id="${examPaperId }-${esm.subjectId }-${index }-D" type="checkbox" name="chooseRight-${esm.subjectId }" value="D"/>&nbsp;<span class="subjectOption">${esm.optionD }</span></p>
										</c:if>
										<%-- 简答题 --%>
										<c:if test="${esm.subjectType == 2 }">
										<p class="aswer-p subject-id-${esm.subjectId }" data-sid="${esm.subjectId }" data-type="textarea" data-val="${examPaperId }-${esm.subjectId }-${index }-"><textarea class="aswer-textarea" name="chooseRight-${esm.subjectId }" style="width:100%;height:100px;" onblur="textareaSubmit(this)"></textarea></p>
										</c:if>
									</div>
									<%--获取 记录已选的试题信息 --%>
									<c:if test="${chooses != null }">
										<c:forEach items="${chooses }" var="choose">
											<c:if test="${choose.subject.subjectId == esm.subjectId }">
												<c:if test="${esm.subjectType == 0 }">
		...
		<div style="width: 38%; float: right;margin-top: -61px;">
			<div style="width:100%; height:63px;text-align: center;">
				...
				<div style="float: right; width: 55%;height: 100%;line-height: 63px; text-align: left;">
					<a href="${path}/student/studentExamSubmit.action?studentId=${sessionScope.loginStudent.studentId }&examPaperId=${examPaperId }&classId=${classId }&gradeId=${gradeId }" type="button" class="btn btn-default btn-sm" onclick="return confirm('确定提交吗?')">提交</a>
					<%--隐藏表单，用于考试结束且考生未手动提交试卷 自动提交 --%>
					<form action="${path}/student/studentExamSubmit.action"method="post" style="display: none;">
