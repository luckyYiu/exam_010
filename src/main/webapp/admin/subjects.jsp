	...
	<div style="text-align: center;">
			...
			<thead>
				<tr>
					...
					<c:if test="${handAdd == null }">
						<th>操作
							&emsp;
							<button type="button" class="btn btn-xs btn-info" onclick="toaddSubject()">添加</button>
						</th>
					</c:if>
				</tr>
			</thead>
			<tbody>
				<c:choose>
					<c:when test="${not empty subjectList }">
						<c:forEach items="${subjectList.result }" var="subject">
							<tr id="tr-${subject.subjectId }">
								...
								<td>
									<c:if test="${handAdd == null }">
										<div class="btn-group">
											<button type="button" class="btn btn-info btn-sm" onclick="toupdateSubject('${subject.subjectId }')">修改</button>
											<button type="button" class="btn btn-danger btn-sm delSubject" onclick="deleteSubject('${subject.subjectId }')" >删除</button>
										</div>
									</c:if>
								</td>
							</tr>
		...
		<div>
			<ul class="pagination">
				...
				<li><a href="${path}/subject/subjectList.action?page=1&handAdd=${handAdd }&examPaperId=${examPaperId }">首页</a></li>
				<c:if test="${subjectList.pageNow-1 > 0 }">
					<li><a href="${path}/subject/subjectList.action?page=${subjectList.pageNow-1 }&handAdd=${handAdd }&examPaperId=${examPaperId }">上一页</a></li>
				</c:if>
				<c:forEach begin="${pageNow }" end="${pageNow+4 }" var="subPage">
					<c:if test="${subPage-5 > 0 }">
						<li><a href="${path}/subject/subjectList.action?page=${subPage-5 }&handAdd=${handAdd }&examPaperId=${examPaperId }">${subPage-5 }</a></li>
					</c:if>
				</c:forEach>
				<c:forEach begin="${subjectList.pageNow }" end="${subjectList.pageNow+5 }" step="1" var="pageNo">
					<!-- 当前页码小于总页数才能显示 -->
					<c:if test="${pageNo <= subjectList.pageCount }">
						<c:if test="${subjectList.pageNow == pageNo }">
							<li class="active"><a href="${path}/subject/subjectList.action?page=${pageNo }&handAdd=${handAdd }&examPaperId=${examPaperId }">${pageNo }</a></li>
						</c:if>
						<c:if test="${subjectList.pageNow != pageNo }">
							<li><a href="${path}/subject/subjectList.action?page=${pageNo }&handAdd=${handAdd }&examPaperId=${examPaperId }" class="pageLink">${pageNo }</a></li>
						</c:if>
					</c:if>
				</c:forEach>
				<c:if test="${subjectList.pageNow+1 <= subjectList.pageCount }">
					<li><a href="${path}/subject/subjectList.action?page=${subjectList.pageNow+1 }&handAdd=${handAdd }&examPaperId=${examPaperId }">下一页</a></li>
				</c:if>
				<li><a href="${path}/subject/subjectList.action?page=${subjectList.pageCount }&handAdd=${handAdd }&examPaperId=${examPaperId }">尾页</a></li>
				<li>
					<a>${subjectList.pageNow }/${subjectList.pageCount }</a>
				</li>
				<li>
					<div style="width:-1%; height:100%;float:right;">
						<form action="${path}/subject/subjectList.action" id="scannerPageForm">
		...
		<%--<div>
			<ul class="pagination">
				...
				<c:if test="${subjectList.pageCount > 1 }">
					<ul class="pagination">
						<li><a href="${path}/subject/subjectList.action?page=1">首页</a></li>
						<c:if test="${subjectList.pageNow-1 > 0 }">
							<li><a href="${path}/subject/subjectList.action?page=${subjectList.pageNow-1 }">上一页</a></li>
						</c:if>
						<c:forEach begin="${subjectList.pageNow }" end="${subjectList.pageNow+4 }" var="subPage">
							<c:if test="${subPage-5 > 0 }">
								<li><a href="${path}/subject/subjectList.action?page=${subPage-5 }">${subPage-5 }</a></li>
							</c:if>
						</c:forEach>
						<c:forEach begin="${subjectList.pageNow }" end="${subjectList.pageNow+5 }" step="1" var="pageNo">
							<c:if test="${pageNo <= subjectList.pageCount }">
								<c:if test="${subjectList.pageNow == pageNo }">
									<li class="active"><a href="${path}/subject/subjectList.action?page=${pageNo }">${pageNo }</a></li>
								</c:if>
								<c:if test="${subjectList.pageNow != pageNo }">
									<li><a href="${path}/subject/subjectList.action?page=${pageNo }" class="pageLink">${pageNo }</a></li>
								</c:if>
							</c:if>
						</c:forEach>
						<c:if test="${subjectList.pageNow+1 <= subjectList.pageCount }">
							<li><a href="${path}/subject/subjectList.action?page=${subjectList.pageNow+1 }">下一页</a></li>
						</c:if>
						<li><a href="${path}/subject/subjectList.action?page=${subjectList.pageCount }">尾页</a></li>
						<li>
							<a>${subjectList.pageNow }/${subjectList.pageCount }</a>
						</li>
						<li>
