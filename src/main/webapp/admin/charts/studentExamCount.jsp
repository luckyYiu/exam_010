...
<script type="text/javascript">
    //加载本班学生数据
    function loadStus() {
        $.getJSON("${path}studentExam/stus.action?tid="+$("#tid").text().trim(), function(json) {
            if(json == "TID-NULL") {
                $("#studentExamCount").html("<h1>请登录后再查看</h1>");
                alert("请登录后再查看");
            } else {
    ...
    function loadShopLine() {
        ...
        $.getJSON("${path}studentExam/avgcounts.action?tid="+$("#tid").text().trim(), function(json) {
            if(json == "TID-NULL") {
    ...
    function loadByStudent(studentId) {
        stuExamInfo.clear();
        stuExamInfo.showLoading({text: "图表正在努力加载中..."});
        $.getJSON("${path}studentExam/stuexam.action?stuId="+studentId, function(json) {
            if(json == "TID-NULL") {
                $("#stuExamInfo").html("<h1>请登录后再查看</h1>");
                alert("请登录后再查看");
            } else {
