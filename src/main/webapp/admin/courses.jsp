			...
			<thead>
				<tr>
					...
					<%--<c:if test="${sessionScope.adminPower == 1 }">--%>
						<th>操作
							&emsp;
							<button type="button" class="btn btn-xs btn-info" onclick="toaddCourse()">添加</button>
						</th>
					<%--</c:if>--%>
				</tr>
			</thead>
						...
						<c:forEach items="${courseList }" var="course">
							<tr>
								...
								<td>
									<%--<c:if test="${sessionScope.adminPower == 1 }">--%>
										<div class="btn-group">
											<button type="button" class="btn btn-info btn-sm" onclick="toupdateCourse('${course.courseId }')">修改</button>
											<button type="button" class="btn btn-danger btn-sm" onclick="deleteCourse('${course.courseId }')">删除</button>
										</div>
									<%--</c:if>--%>
								</td>
							</tr>
