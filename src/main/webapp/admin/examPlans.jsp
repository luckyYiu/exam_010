		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th>操作
						&emsp;
						<a type="button" onclick="toaddExamPlan()" class="btn btn-xs btn-info" target="right">添加</a>
					</th>
				</tr>
			</thead>
			<tbody>
				<c:choose>
					<c:when test="${not empty examPlanList }">
						<c:forEach items="${examPlanList }" var="plan">
							<tr>
								...
								<td>
									<div class="btn-group">
										<input type="button" class="btn btn-info btn-sm" onclick="toupdateExamPlan('${plan.examPlanId}')" value="修改"/>
										<input type="button" onclick="deleteExamPlan('${plan.examPlanId}')" class="btn btn-danger btn-sm" value="移除"/>
									</div>
								</td>
							</tr>
						</c:forEach>
