		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th>所属年级</th>
					<th>操作
						<button type="button" class="btn btn-xs btn-info" onclick="toaddExamPaper()">添加</button>
					</th>
				</tr>
			</thead>
			<tbody>
					...
					<c:when test="${not empty examPapers }">
						<c:forEach items="${examPapers }" var="ep">
							<tr class="subject_info">
								...
								<td>
									<div class="btn-group">
										<button type="button" class="btn btn-info btn-sm" onclick="toupdateExamPaper('${ep.examPaperId}')">修改</button>
										<button type="button" class="btn btn-danger btn-sm" onclick="deleteExamPaper('${ep.examPaperId}')">删除</button>
